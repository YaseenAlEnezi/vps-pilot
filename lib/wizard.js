...
/**
 * Interactive directory browser. Loops until user selects "Select this folder".
 * @param {string} [initialDir] - directory to start in (default: os.homedir())
 * @returns {Promise<string>} absolute path of selected directory
 */
async function browseForPath(initialDir) {
  let currentDir = path.resolve(initialDir || os.homedir());

  for (;;) {
    const choices = [
      { name: '📁 Select this folder', value: SELECT_FOLDER },
    ...
    try {
      ...
      const subdirs = entries
        ...
        .filter((d) => {
          ...
          try {
            fs.readdirSync(path.join(currentDir, d.name), { withFileTypes: true });
            return true;
          } catch (err) {
      ...
      choices.push(...subdirs);
    } catch (err) {
      if (err.code === 'EACCES' || err.code === 'EPERM') {
        console.log(chalk.yellow(`Cannot read directory: ${currentDir}`));
    ...
    const { action } = await inquirer.prompt([
      {
        type: 'list',
        name: 'action',
        message: `Current path: ${currentDir}\nChoose folder:`,
        choices,
        pageSize: 15,
      },
    ]);

    if (action === SELECT_FOLDER) return currentDir;
    if (action === GO_UP) {
      const parent = path.dirname(currentDir);
    ...
    currentDir = path.join(currentDir, action);
  }
}

/**
 * Returns the single "name" question (used before path browser).
 */
function nameQuestion(existing = {}) {
  return [
    {
      type: 'input',
      name: 'name',
...
/**
 * Build per-project questions (excluding name and path). Used after name + path browser.
 * @param {Object} [existing] - existing project for defaults
 * @param {Object} [answersSoFar] - { name } for default pm2Name
 * @returns {Array}
 */
function projectQuestionsRest(existing = {}, answersSoFar = {}) {
  return [
    {
      type: 'list',
      name: 'type',
    ...
    },
    {
      ...
      default: existing.pm2Name || (answersSoFar.name && slugify(answersSoFar.name)) || (existing.name && slugify(existing.name)) || '',
  ...
  while (addAnother) {
    if (configChoice === 'update') {
      ...
      if (projectList.length === 0) {
        ...
        const answersName = await inquirer.prompt(nameQuestion(existing));
        const pathSelected = await browseForPath(existing.path || os.homedir());
        const answersRest = await inquirer.prompt(projectQuestionsRest(existing, answersName));
        const answers = { ...answersName, path: pathSelected, ...answersRest };
        const project = {
          ...existing,
          ...answers,
          id: projectId,
    ...
    const answersName = await inquirer.prompt(nameQuestion());
    const pathSelected = await browseForPath(os.homedir());
    const answersRest = await inquirer.prompt(projectQuestionsRest({}, answersName));
    const answers = { ...answersName, path: pathSelected, ...answersRest };
    const project = {
      ...answers,
      port: parseInt(answers.port, 10),
